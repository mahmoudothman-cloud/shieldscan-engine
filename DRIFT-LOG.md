...
**Live reproduction — ghost #14, the first with a known cause and timestamp (2026-08-17).** Scan `f82bdac0-a61b-41fb-8004-40c54ba6532d`: `recon`, `zap`, `wapiti`, `nikto`, `nmap` and `sslyze` all completed; **`nuclei` stranded at `queued`**; Redis queues empty. The worker was killed mid-scan while testing the container reaper. Every ghost before this was reconstructed after the fact; this one has a known cause, so the mechanism above is confirmed by reproduction and not only by inference. Running total: **14 stranded jobs / 11 non-terminal scans**.

**The zap bias in the fossil record is shape, not engine.** The earlier reading — "6 of 7 are zap, the slowest engine" — named the wrong invariant. The stranded job is **whichever engine happens to be slowest for that scan profile at that moment**, because that is the one still running when everything else has finished. In `f82bdac0` zap completed in **56.0s** and `nuclei`, the ~340s engine here, was the one left holding the bag. Any change to engine timings — a tuned wordlist, a new template set, a slower target — moves the stranding to a different engine. A janitor keyed on specific engine names would be built on the coincidence rather than the cause.

**Ghost #15, and the discriminator catching a miscount within hours of being written down (2026-08-18).** Scan `2937e486`, worker `kill -9`'d during reaper testing. Read by database status alone this looked like **six** stranded phase-2 jobs; the payload test says **one**. Job timings settle it: `nikto` (9.1s), `sslyze` (1.2s), `wapiti` (17.5s), `nmap` (18.4s) and `zap` (55.9s) all completed between **22:48:27 and 22:49:14** — before the kill, not after a later restart — while `nuclei` alone was still running and was lost. Running total **15 stranded jobs / 12 non-terminal scans**. Third instance of the slowest-engine shape, and `nuclei` again.

**Exposure is bounded by worker concurrency, not by one job per death.** `SHIELDSCAN_WORKER_CONCURRENCY=5`, and this scan shows all five slots genuinely occupied at once — five engines finishing inside a 47-second window while a sixth ran on. A kill landing anywhere in that window strands every job then in flight, so the worst case per death is the concurrency setting, not one. This particular kill happened to land after the five had finished, so it took one. Sizing any reclaim mechanism off "one job per worker death" would under-build it by 5×.

**Status alone cannot identify a ghost — the payload is the discriminator.** Confirmed live: at 2026-08-18 22:31 scan `2937e486` sat at `queued` in PostgreSQL exactly like the 14 ghosts, but its payload was still present in `shieldscan:queue:normal` (LLEN 1, stable across samples) — a legitimately pending job awaiting pickup, not a ghost. The test that separates them is **`queued` in the database AND payload absent from Redis**, and any future janitor must apply it: a sweeper that fails jobs on DB status alone would kill live queued work. The same check also caught an unrelated live condition — the worker process was down, so nothing was consuming the queue.
...
`bec0ff6` is verified on all three of its paths against real Docker, not only against fakes: **labelling at create** (`distracted_thompson` carrying `io.shieldscan.worker-id` on a live `ContainerCreate`), **graceful-shutdown removal** (the 2026-08-17 03:12 `exit_code=0` drain sequence), and the **SIGKILL orphan sweep** — `kill -9`, container outliving its owner, a deliberate 70s wait for the 60s heartbeat TTL to expire (`KEYS shieldscan:workers:*` confirmed empty first, so the liveness guard made a real dead-vs-live decision rather than being bypassed), then on restart: `reaped orphan container from dead worker container_id=4f5b4ff244cb owner_worker_id=shie-4a652e46 pool=nmap`, `removed=1`.
