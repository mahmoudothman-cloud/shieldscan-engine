...
func TestWaitForReady_ImmediateSuccess(t *testing.T) {
	...
	err := waitForReady(context.Background(), server.URL, "/ready", 200, 2*time.Second, 10*time.Millisecond, "")
...
func TestWaitForReady_TimeoutOnUnreachable(t *testing.T) {
	...
	err := waitForReady(context.Background(), "http://127.0.0.1:1", "/ready", 200, 100*time.Millisecond, 20*time.Millisecond, "")
	require.Error(t, err)
...
func TestWaitForReady_StatusMismatchKeepsPolling(t *testing.T) {
	...
	err := waitForReady(context.Background(), server.URL, "/", 200, 2*time.Second, 5*time.Millisecond, "")
	require.NoError(t, err)
...
func TestWaitForReady_ContextCanceled(t *testing.T) {
	...
	err := waitForReady(ctx, server.URL, "/", 200, time.Hour, 10*time.Millisecond, "")
	require.Error(t, err)
}
...
func TestWaitForReady_ZeroExpectedStatus_DefaultsTo200(t *testing.T) {
	...
	defer server.Close()
	err := waitForReady(context.Background(), server.URL, "/", 0, time.Second, 10*time.Millisecond, "")
	require.NoError(t, err)
}
...
func TestWaitForReady_ZeroPollInterval_DefaultsApplied(t *testing.T) {
	...
	err := waitForReady(context.Background(), server.URL, "/", 200, 5*time.Second, 0, "")
	require.NoError(t, err)
}
