...
// + shieldscan-api 824853c orchestrator dispatch+audit): SignedFetchURL
// is the Q1 (β) sibling-field carrying a pre-signed R2 GetObject URL
// generated by the orchestrator at scan-dispatch time. Engine consumer
// (MobSF) prefers SignedFetchURL via plain HTTP GET when present + non-
// empty; falls back to UploadRef r2:// path via worker-side R2 SDK per
// Q3 (a) backward-compat migration window. omitempty tag preserves
// wire compat with older orchestrator that emits payload without the
// field (engine falls back to UploadRef). Migration close
// (forward-pinned to "Begin MobSF R2 migration-close task" per
// Q7-refined) removes UploadRef emission once SignedFetchURL adoption
// empirically stable.
type JobMobileConfig struct {
	UploadRef      string `json:"upload_ref"`
	SignedFetchURL string `json:"signed_fetch_url,omitempty"`
